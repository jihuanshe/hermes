...
_PLATFORMS = [
    ...
    {
        "key": "feishu_cli",
        "label": "Feishu / Lark (lark-cli)",
        "emoji": "🪽",
        "token_var": "FEISHU_CLI_ENABLED",
        "setup_instructions": [
            "1. Install lark-cli: https://github.com/larksuite/cli",
            "2. Run 'lark-cli config init' to configure App ID + App Secret (bot identity)",
            "3. Enable Bot capability in Feishu Open Platform console",
            "4. Enable im:message:receive_as_bot scope",
            "5. Events & Callbacks → select 'Use long connection to receive events'",
            "6. Set FEISHU_CLI_ENABLED=true to activate",
            "7. No 'auth login' needed — bot identity uses config init only",
        ],
        "vars": [
            {"name": "FEISHU_CLI_ENABLED", "prompt": "Enable Feishu CLI adapter (true/false)", "password": False,
             "help": "Set to 'true' to enable the lark-cli based Feishu adapter."},
            {"name": "FEISHU_BOT_OPEN_ID", "prompt": "Bot open_id (optional, for self-message filtering)", "password": False,
             "help": "The bot's own open_id (ou_xxx) to prevent reply loops."},
            {"name": "FEISHU_ALLOWED_USERS", "prompt": "Allowed user IDs (comma-separated, or empty)", "password": False,
             "is_allowlist": True,
             "help": "Restrict which Feishu/Lark users can interact with the bot."},
            {"name": "FEISHU_CLI_HOME_CHANNEL", "prompt": "Home chat ID (optional, for cron/notifications)", "password": False,
             "help": "Chat ID for scheduled results and notifications."},
        ],
    },
    {
